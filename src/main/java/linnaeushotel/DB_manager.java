		...
		try{
			...
			while(i<g.getReservations().size()&&g.getReservations()!=null){
				...
				DBObject roomObject= new BasicDBObject("_id",room.getId())
						...
						.append("RoomType",room.getType().toString())
						.append("RoomQuality", room.getQuality().toString())
						.append("Location", room.getLocation().toString())
						.append("smoker", room.isSmoker())
				...
				DBObject test = new BasicDBObject("startDate", g.getReservations().get(i).getStartDate())
						...
						.append("room", roomObject)
						.append("price", g.getReservations().get(i).getPrice())
		...
		try{
			...
			while(cursor.hasNext()){
				...
				for (int i = 0; i < reservationList.size(); i++) {
					...
					Room room = new Room(((int)roomObject.get("roomNumber")),b,a,c,(boolean)roomObject.get("smoker"),(boolean)roomObject.get("reserved"));
...
public void insertReservation(Reservation g){
	...
	try{
		...
		DBObject reservation = new BasicDBObject("_id",g.getId())
			.append("startDate",g.getStartDate())
